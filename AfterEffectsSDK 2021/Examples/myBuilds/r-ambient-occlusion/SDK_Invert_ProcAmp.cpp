	...
	PF_LayerDef		*output)
{
	...
	AEFX_CLR_STRUCT(def);
	PF_ADD_TOPIC("Ambient Occlusion", TOPIC_ADD_AO_SETTINGS);
	...
	AEFX_CLR_STRUCT(def);
	PF_END_TOPIC(TOPIC_END_AO_SETTINGS);
	...
	PF_PreRenderExtra	*extraP)
{
	...
	if (infoP) {
		...
		ERR(PF_CHECKOUT_PARAM(in_dataP, SLIDER_INTENSITY, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->intensity = static_cast<float>(cur_param.u.fs_d.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, SLIDER_THRESHOLD, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->threshold = static_cast<float>(cur_param.u.fs_d.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, SLIDER_XYRADIUS, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->xyRadius = static_cast<float>(cur_param.u.fs_d.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, SLIDER_ZRADIUS, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->zRadius = static_cast<float>(cur_param.u.fs_d.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, SLIDER_SAMPLES, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->samples = static_cast<int>(cur_param.u.fs_d.value);
	...
	const char* inHostVersion)
{
	...
	result = PF_REGISTER_EFFECT(
		...
		"r-ambient-occlusion", // Name
		"ADBE r-ambient-occlusion", // Match Name
		"Rikki", // Category
