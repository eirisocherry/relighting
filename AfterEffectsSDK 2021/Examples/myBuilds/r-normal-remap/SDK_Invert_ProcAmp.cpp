	...
	PF_LayerDef		*output)
{
	...
	AEFX_CLR_STRUCT(def);
	PF_ADD_CHECKBOXX("Input Is Normalized", TRUE, 0, CHECKBOX_INPUT_IS_NORMALIZED_TOGGLE);

	AEFX_CLR_STRUCT(def);
	PF_ADD_POPUP("X", 6, 4, "X|Y|Z|-X|-Y|-Z", POPUP_X);

	AEFX_CLR_STRUCT(def);
	PF_ADD_POPUP("Y", 6, 6, "X|Y|Z|-X|-Y|-Z", POPUP_Y);

	AEFX_CLR_STRUCT(def);
	PF_ADD_POPUP("Z", 6, 5, "X|Y|Z|-X|-Y|-Z", POPUP_Z);

	AEFX_CLR_STRUCT(def);
	PF_ADD_CHECKBOXX("Normalize Output", TRUE, 0, CHECKBOX_NORMALIZE_OUTPUT_TOGGLE);
	...
	PF_PreRenderExtra	*extraP)
{
	...
	if (infoP) {
		...
		ERR(PF_CHECKOUT_PARAM(in_dataP, CHECKBOX_INPUT_IS_NORMALIZED_TOGGLE, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->inputIsNormalizedToggle = static_cast<bool>(cur_param.u.bd.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, POPUP_X, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->x = static_cast<int>(cur_param.u.pd.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, POPUP_Y, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->y = static_cast<int>(cur_param.u.pd.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, POPUP_Z, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->z = static_cast<int>(cur_param.u.pd.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, CHECKBOX_NORMALIZE_OUTPUT_TOGGLE, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->normalizeOutputToggle = static_cast<bool>(cur_param.u.bd.value);
	...
	const char* inHostVersion)
{
	...
	result = PF_REGISTER_EFFECT(
		...
		"r-normal-remap", // Name
		"ADBE r-normal-remap", // Match Name
		"Rikki", // Category
