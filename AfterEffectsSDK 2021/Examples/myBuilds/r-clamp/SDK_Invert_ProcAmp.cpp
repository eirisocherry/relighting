	...
	PF_LayerDef		*output)
{
	...
	AEFX_CLR_STRUCT(def);
	PF_ADD_CHECKBOXX("Keep Alpha", TRUE, 0, CHECKBOX_KEEP_ALPHA);
	
	AEFX_CLR_STRUCT(def);
	PF_ADD_CHECKBOXX("Clamp", TRUE, 0, CHECKBOX_CLAMP);
	...
	PF_PreRenderExtra	*extraP)
{
	...
	if (infoP) {
		...
		ERR(PF_CHECKOUT_PARAM(in_dataP, CHECKBOX_KEEP_ALPHA, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->keepAlphaToggle = static_cast<bool>(cur_param.u.bd.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, CHECKBOX_CLAMP, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->clampToggle = static_cast<bool>(cur_param.u.bd.value);
