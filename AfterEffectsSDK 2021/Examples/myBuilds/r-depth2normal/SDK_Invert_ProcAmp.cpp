	...
	PF_LayerDef		*output)
{
	...
	AEFX_CLR_STRUCT(def);
	PF_ADD_TOPIC("Normal", TOPIC_ADD_NORMAL);

	AEFX_CLR_STRUCT(def);
	PF_ADD_CHECKBOXX("Improve", TRUE, 0, CHECKBOX_NORMAL_IMPROVE);

	AEFX_CLR_STRUCT(def);
	PF_ADD_CHECKBOXX("Normalize", TRUE, 0, CHECKBOX_NORMAL_NORMALIZE);

	AEFX_CLR_STRUCT(def);
	PF_END_TOPIC(TOPIC_END_NORMAL);
	...
	AEFX_CLR_STRUCT(def);
	PF_ADD_TOPIC("Smooth Normal", TOPIC_ADD_SMOOTH_NORMAL);

	AEFX_CLR_STRUCT(def);
	PF_ADD_CHECKBOXX("Smooth", TRUE, 0, CHECKBOX_SMOOTH);
	...
	AEFX_CLR_STRUCT(def);
	PF_END_TOPIC(TOPIC_END_SMOOTH_NORMAL);
	...
	PF_PreRenderExtra	*extraP)
{
	...
	if (infoP) {
		...
		ERR(PF_CHECKOUT_PARAM(in_dataP, CHECKBOX_NORMAL_IMPROVE, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->improve = static_cast<bool>(cur_param.u.bd.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, CHECKBOX_NORMAL_NORMALIZE, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->normalize = static_cast<bool>(cur_param.u.bd.value);

		// Smooth Normal

		ERR(PF_CHECKOUT_PARAM(in_dataP, CHECKBOX_SMOOTH, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->smooth = static_cast<bool>(cur_param.u.bd.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, SLIDER_SMOOTH_RADIUS, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->radius = static_cast<int>(cur_param.u.fs_d.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, SLIDER_SMOOTH_THRESHOLD, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->normalThreshold = static_cast<float>(cur_param.u.fs_d.value);

		ERR(PF_CHECKOUT_PARAM(in_dataP, SLIDER_SMOOTH_DEPTH_WEIGHT, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
		infoP->depthWeight = static_cast<float>(cur_param.u.fs_d.value);
	...
	InvertProcAmpParams		*infoP)
{
	...
	ERR(gpu_suite->CreateGPUWorld(in_dataP->effect_ref,
		...
		&second_buffer));
	void* second_mem = 0;
	ERR(gpu_suite->GetGPUWorldData(in_dataP->effect_ref, second_buffer, &second_mem));

	PF_EffectWorld* third_buffer;
	ERR(gpu_suite->CreateGPUWorld(in_dataP->effect_ref,
		extraP->input->device_index,
		input_worldP->width,
		input_worldP->height,
		...
		&third_buffer));
	void* third_mem = 0;
	ERR(gpu_suite->GetGPUWorldData(in_dataP->effect_ref, third_buffer, &third_mem));
		...
		CL_ERR(clEnqueueNDRangeKernel(
			...
			if (infoP->smooth) {
				...
				firstPassCUDA(
					...
					second_buffer->rowbytes / bytes_per_pixel,
					second_buffer->width,
					second_buffer->height,
					(pixel_format != PF_PixelFormat_GPU_BGRA128),
				...
				secondPassCUDA(
					...
					third_buffer->rowbytes / bytes_per_pixel,
					third_buffer->width,
					third_buffer->height,
					(pixel_format != PF_PixelFormat_GPU_BGRA128),
	...
	ERR(gpu_suite->DisposeGPUWorld(in_dataP->effect_ref, second_buffer));
	ERR(gpu_suite->DisposeGPUWorld(in_dataP->effect_ref, third_buffer));
	...
	bool					isGPU)
{
	...
	if (infoP) {
		...
		PF_PixelFormat	normal_pixel_format = PF_PixelFormat_INVALID;
	...
	const char* inHostVersion)
{
	...
	result = PF_REGISTER_EFFECT(
		...
		"r-depth2normal", // Name
		"ADBE r-depth2normal", // Match Name
		"Rikki", // Category
